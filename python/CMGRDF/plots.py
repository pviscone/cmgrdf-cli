...
class PlotSetPrinter:
    ...
    def printSet(self, plots : MultiReport, path : str, ncpu = None, **options) -> None:
        ...
        for plotKey, plot in plots:
            pool_data.append((plot, path.format(**plotKey), opts))
...
def _printPlot(plot : PlotResult, path : str,  opts) -> None:
    ...
    totalError = doShadedUncertainty(total) if opts.showErrors and not is2D else None
    ...
    if opts.extraLabel:
        if plot.getOpt('legend', 'TR') == "TL":
            addLabel(c1, opts.extraLabel, .68, .855, .9, .895, align=32, textSize=smallTextSize)
        else:
            addLabel(c1, opts.extraLabel, .23, .855, .6, .895, align=12, textSize=smallTextSize)
    doLegend(p1, plot, total, totalError, opts, locals())
    addLabels(p1, hasExpo=total.GetMaximum() > 9e4 and not islog, textSize=smallTextSize, opts=opts, doWide=doWide, lumi=plot.lumi)
    if outputTDir:
        outputTDir.WriteTObject(c1)
    if opts.showRatio:
        nums = data
        den = total
        doRatioHists(p2, plot, nums, den, opts, locals())
...
def addLabels(c1, opts : Options, hasExpo=False, textSize=0.033, xoffs=0, doWide=False, lumi=None) -> None:
    ymin, ymax = .955, .995
    if opts.topLeftText not in ['', None]:
        addLabel(c1, opts.topLeftText % dict(lumi=lumi),
    ...
    if opts.topRightText not in ['', None]:
        addLabel(c1, opts.topRightText % dict(lumi=lumi),
                        (0.5 if doWide else .58) + xoffs, ymin, .98 + xoffs, ymax,
                        align=32, textSize=textSize)

def doLegend(c1, plot, total, totalError, opts, locvars) -> None:
...
def doRatioHists(pane, plot, nums, den, opts, locvars):
