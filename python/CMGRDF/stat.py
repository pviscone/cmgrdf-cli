...
import ROOT
from CMGRDF.histoWithNuisances import HistoWithNuisances, listAllNuisances, mergePlots
...
class DatacardWriter(object):
    ...
    def makeCards(self, plots : MultiReport, plotKey : MultiKey, outname : str, **options):
        ...
        for key, plot in plots:
            if not plotKey.isSuperSet(key): continue
            ...
            for p,h in plot.histos:
                ...
                if p.isData:
                    ...
                    if h.Integral() > opts.threshold: signals.append((p,h))
                else:
                    if h.Integral() > opts.threshold: backgrounds.append((p,h))
                if p.name in byname:
            ...
            if opts.asimov:
                ...
                if len(data_obs) != 1: raise RuntimeError("Can't choose process for data_obs among %s" % ([p.name for (p,h) in data_obs]))
            ...
            for i,(s,h) in enumerate(signals):
                procs.append(s.name); iproc[s.name] = i-len(signals)+1
            for i,(b,h) in enumerate(backgrounds):
                procs.append(b.name); iproc[b.name] = i+1
            systs = {}
