...
from CMGRDF.flow import AddWeight, AddWeightUncertainty, Cut, FlowStep, Define, Flow, Target, Yield
...
class Processor(object):
    ...
    def _growBranch(self, source : Source, flow : Flow, treeName="Events", verbose=False):
        ...
        wdef = Define("weight", "*".join(tree.weights) if tree.weights else "1")
        tree = tree.maybeBranch(wdef, verbose=verbose)
        return tree
    ...
    def book(self, processes : Sequence[Process], lumi, flows : Union[Flow,Sequence[Flow]], targets : Union[Target,Sequence[Target]], eras=None, taskName="", withUncertainties=False, logPerformance=True):
        ...
        if isinstance(targets,Target): targets = [targets]
        futuresToVary = []
        for flow in flows:
            for era in eras:
                ...
                for proc in processes:
                    ...
                    for sample in proc.samples:
                        ...
                        for t in targets:
                            if t.mcOnly and not sample.isMC: continue
            ...
            for era in eras:
                for proc in processes:
                    ...
                    for sample in proc.samples:
                        ...
                        branch = self._growBranch(src, [], verbose=verbose)
                        for step in flow.steps:
                            branch = branch.maybeBranch(step, verbose=verbose)
                            if type(step) in (Cut, AddWeight, AddWeightUncertainty):
                                if (cutNames == None) or (step.name in cutNames):
                                    wdef = Define("weight", "*".join(branch.weights) if branch.weights else "1")
                                    wbranch = branch.maybeBranch(wdef, verbose=verbose)
    ...
    def runPlots(self, mergeEras=False, mergeSamples=True, logPerformance=True, **kwargs):
        ...
        for plotKey, (proc, sample, plot, hraw, vars) in rawReport:
            ...
            for nuis in sample.normUncertainties+proc.normUncertainties:
                if nuis.eras and plotKey.era not in nuis.eras: continue
    ...
    def runYields(self, mergeEras=False, mergeSamples=True, logPerformance=True, **kwargs):
        ...
        plots = MultiReport()
        for plotKey, (proc, sample, plot, hraw, vars) in rawReport:
            if not isinstance(plot,Yield): continue # there may be other stuff depending on book
        ...
        if logPerformance: print("Merged %d yields in %.3fs" % (len(merged),time.perf_counter()-t0))
        return merged        